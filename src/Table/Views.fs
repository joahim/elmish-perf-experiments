...
let renderTable model =
    ...
    let sortRows sortOrder rows =
        ...
        | Ascending ->
            rows
            |> Array.sortBy (fun (key, row) -> key)
        | Descending ->
            rows
            |> Array.sortByDescending (fun (key, row) -> key)
    ...
    table
        ...
        [ tbody
            [ ]
            [ renderRows model.SortOrder model.Rows ] ]

let view model dispatch =
    div
        [ ]
        [ button
            [ ClassName "button" ; OnClick (fun _ -> dispatch ChangeSortOrder) ]
            [ str "Sort" ]
          renderTable model ]
